...
void WebServer::eventListen()
{
    ...
    add_fd(m_epollfd, m_listenfd, false, m_LISTENTrigmode);
    http_conn::m_epollfd = m_epollfd;

    ret = socketpair(PF_UNIX, SOCK_STREAM, 0, m_pipefd);
    assert(ret != -1);
    utils.set_nonblock(m_pipefd[1]);
    utils.add_fd(m_epollfd, m_pipefd[0], false, 0);

    Utils::sig_pipefd = m_pipefd;
...
void WebServer::eventLoop()
{
    ...
    while (stop_server)
    {
        ...
        for(int i = 0;i < number; ++i)
        {
            ...
            {
                bool flag = deal_with_signal(timeout, 
                stop_server);
                
                if(false == flag)
                    LOG_ERROR("%s", "deal_signal failure");
            }
